...
function is31Flavors(flavorsList){
    if (flavorsList.length === 31) {
        return true;
    }
    else if (flavorsList.length !== 31) {
        return false;
    }
}
...
and should return a new array that is identical to the original array. You can name the new array however you'd like. */

function copy(originalList){
...
function filterByWord(flavorList, flavor){
    ...
    for (let i = 0; i < flavorList.length; i++){
      if (flavorList[i].includes(flavor)){
        filteredList.push(flavorList[i]);
      }
    }
    return filteredList;
  }
